...
#include <unistd.h> 
#include <pthread.h>
pthread_t *philosophers; pthread_mutex_t *forks;
...
pthread_mutex_lock(forks + (first>second?second:first)); pthread_mutex_lock(forks + (first<second?second:first)); eat(i);
pthread_mutex_unlock(forks+first); pthread_mutex_unlock(forks+second);
}
 return NULL;

}
int main(void){
 int i,err; srand(time(NULL));
